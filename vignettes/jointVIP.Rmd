...
The `jointVIP` package uses `pilot_df` and `analysis_df` to denote datasets used. The `analysis_df` is the matching/weighting dataset of interest while `pilot_df` contains the external controls not used in the analysis. The `pilot_df` is used to help inform the outcome correlation and compare the omitted variable bias-based (cross-sample) standardized mean difference. If external data is not available, one may choose to sacrifice a portion of the analysis controls to form the `pilot_df`.
