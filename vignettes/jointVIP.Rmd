...
my.dist <- build.dist.struct(z = analysis_df[[treatment]],
                             X = analysis_df[, maha.vars])
# compute match
match.out <- rcbalance(
  my.dist,
  fb.list = list(l1, l2),
  treated.info = analysis_df[analysis_df[[treatment]] == 1, ],
  control.info = analysis_df[analysis_df[[treatment]] == 0, ],
  exclude.treated = T
)

rcmatch_df = analysis_df[c(which(analysis_df[[treatment]] == 1)[as.numeric(row.names(match.out$matches))],
                           which(analysis_df[[treatment]] == 0)[match.out$matches]),
...
### Post-match assessment with `post_jointVIP` object
