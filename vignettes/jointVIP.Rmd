...
# weighting methods shown in example
library(WeightIt)
```
...
### Refined covariate balance matching

The refined covariate balance matching (rcbalance) is more involved [@pimentel2016large]; see @pimentel2015large for details. Specifically, fine balance needs to be specified in nominal nested sets of the top most desired variables to balance. First, the most stringent balance is performed for `log_re75`, then `log_re74` is specified as well. 

Since the `log_re75` and `log_re74` are both numeric, slight variable transformation into 6 groups: those with 0 and all quintiles of the remaining earnings. 
...
Both methods yielded satisfactory results based on desired bias tolerance.
...
```{r post_rc, dpi=300, fig.asp = 0.75, fig.width = 6, out.width = "80%", fig.align = "center", message=FALSE}
...
plot(post_rcmatch_jointVIP, plot_title = "Post-match jointVIP using rcbalance matching")
```
...
plot(post_optwt_jointVIP, plot_title = "Post-weighting jointVIP using optweight matching")
```
