---
title: "additional_options"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{additional_options}
...
See the `vignette("jointVIP")` to get started on how to use `jointVIP` package. Using the same data sets, this vignette's main purpose is to demonstrate other options that are available. 
...
l1 <- c("re75_cut")
l2 <- c(l1, "re74_cut")
analysis_df_wcut <- analysis_df
analysis_df_wcut$re75_cut <- cut(
  analysis_df$log_re75,
  breaks =
    quantile(analysis_df$log_re75,
             c(0,
               seq(
                 mean((analysis_df$log_re75 == 0)),
                 1,
                 length.out = 5
               ))),
  include.lowest = TRUE
...
analysis_df_wcut$re74_cut <- cut(
  analysis_df$log_re74,
  breaks =
    quantile(analysis_df$log_re74,
             c(0,
               seq(
                 mean(analysis_df$log_re74 == 0),
                 1,
                 length.out = 5
               ))),
  include.lowest = TRUE
...
my.dist <- build.dist.struct(z = analysis_df_wcut[[treatment]],
                             X = analysis_df_wcut[, maha.vars])
# compute match
match.out <- rcbalance(
  my.dist,
  fb.list = list(l1, l2),
  treated.info = analysis_df_wcut[analysis_df_wcut[[treatment]] == 1, ],
  control.info = analysis_df_wcut[analysis_df_wcut[[treatment]] == 0, ],
  exclude.treated = T
)

rcmatch_df = analysis_df_wcut[c(which(analysis_df_wcut[[treatment]] == 1)[as.numeric(row.names(match.out$matches))],
                           which(analysis_df_wcut[[treatment]] == 0)[match.out$matches]),
                         c(treatment, outcome, covariates)]


post_rcmatch_jointVIP = create_post_jointVIP(new_jointVIP,
                                             post_analysis_df = rcmatch_df)
```
...
* The `smd` parameter allows only `standard` or `OVB-based` options. While the OVB-based, stands for omitted variable bias-based, standardized mean difference (SMD) is used as default, a pooled version of the standardized mean difference (standard) can be specified. The `standard` option is the typical option used in balance tables and Love plots that uses both treated and control variances from the analysis data set to construct the SMD. 
...
* `bias_curve_cutoffs`: draws bias curves by the specifications. This is only used when `smd` is specified as `OVB-based`.
...
* `OVB_curves`: TRUE (default) draws the omitted variable bias curves, FALSE suppresses the bias curves. If `bias_curve_cutoffs` also specified, `OVB_curves` takes priority. This is only used when `smd` is specified as `OVB-based`.
* `add_var_labs`: TRUE (default) adds variable labels. This suppresses all `label_cut` inputs if specified FALSE.
* `expanded_y_curvelab`: if one wishes to expand the y-axis, the bias curve labels don't automatically get updated. So, this allows the fine-tuning of the bias-curve labels. Typically this is used under the hood for bootstrap version of the plot. However, user can specify this if they wish. This is only used when `smd` is specified as `OVB-based`.

# Post-analysis parameters examples

The same variables are specified in the `vignette("jointVIP")`; here we choose refined covariate balance matching example to demonstrate the additional parameters. 

## Demonstration for additional options in `summary()` and `print()`

```{r sumnprint_post, dpi=300, fig.asp = 0.75, fig.width = 6, out.width = "80%", fig.align = "center", message=FALSE}
# get_post_measures(post_rcmatch_jointVIP, smd = 'OVB-based')

summary(post_rcmatch_jointVIP,
        use_abs = FALSE,
        bias_tol = 0.01,
        post_bias_tol = 0.001)

print(post_rcmatch_jointVIP,
      bias_tol = 0.001)

plot(post_rcmatch_jointVIP, 
...
* `summary()` function adds text output of post-match results in addition to pre-match results. The `post_bias_tol` specifically is a `summary()` parameter that outputs post_bias tolerance for text comparison.
